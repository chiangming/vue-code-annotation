...
// 针对'create', 'activate', 'update', 'remove', 'destroy'创建或者合并回调函数cb
export function createPatchFunction(backend) {
   ...
   *  -> 调用平台 DOM 的操作去创建一个占位符元素 vnode.elm = document.createElement(tag, vnode)
   *  -> 迭代调用
   *     -> createChildren 方法去创建子元素
   *     -> invokeCreateHooks 方法执行所有的 create 的钩子
   *     -> insert方法将 vnode push 到 insertedVnodeQueue 中
   *************************************************************************************/
  function createElm(
    vnode,
    insertedVnodeQueue,
  ...
  //hook 定义在vdom/create-component.js中
  ...
  return function patch(oldVnode, vnode, hydrating, removeOnly) {
    ...
    if (isUndef(oldVnode)) {
      ...
      const isRealElement = isDef(oldVnode.nodeType) // 判断是否为真实dom节点
      if (!isRealElement && sameVnode(oldVnode, vnode)) {
        ...
        if (isRealElement) {
          ...
          // 将oldValue转换为VNode对象，在调用createElm方法
